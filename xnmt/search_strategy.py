...
class TransformerBeamSearch(SearchStrategy):
  ...
  def generate_output(self, decoder, encodings, trg_embedder, src_length=None, forced_trg_ids=None):
    ...
    ss_embeddings = trg_embedder.embed(ss)
    dec_input_embeddings = ExpressionSequence([ss_embeddings])
      ...
      for hyp in active_hyp:
        ...
        if length > 0:  # don't feed in the initial start-of-sentence token
          if hyp.id_list[-1] == Vocab.ES:
            completed_hyp.append(hyp)
            continue
          dec_input_embeddings = ExpressionSequence(dec_input_embeddings.expr_list + [trg_embedder.embed(hyp.id_list[-1])])

        temp = decoder.get_scores(encodings, dec_input_embeddings)
        temp2 = dy.pick_batch_elem(temp, length)

        score = dy.log_softmax(temp2).npvalue()
        if forced_trg_ids is None:
          top_ids = np.argpartition(score, max(-len(score), -self.beam_size))[-self.beam_size:]
        else:
          top_ids = [forced_trg_ids[length]]
