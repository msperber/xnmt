...
class LossBuilder(object):
  def __init__(self, init_loss=None, normalize_loss=True):
  ...
  def compute(self):
    if self.normalize_loss:
      return dy.sum_batches(dy.esum([x*(1.0/x.dim()[1]) for x in self.loss_values.values()]))
    else:
      return dy.sum_batches(dy.esum(list(self.loss_values.values())))

  def value(self):
    # TODO: should normalize here as well?
